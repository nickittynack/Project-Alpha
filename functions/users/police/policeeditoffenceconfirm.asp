...
<%
	...
	offname = UCase(Request.Form("offname")) 
	offsub = UCase(Request.Form("offsuburb"))	
	offdate = UCase(Request.Form("offdate"))
	...
	stat = UCase(Request.Form("status"))	
	...
	id = Request.Cookies("policeeditoffenceid")	
	
	sql="UPDATE offence SET "
	sql=sql & "offencename='" & offname & "',"
	sql=sql & "status='" & stat & "',"
	sql=sql & "offencesuburb='" & offsub & "',"
	sql=sql & "offencedate='" & offdate & "',"
	
	sql=sql & "firstname='" & fn & "',"
	sql=sql & "lastname='" & ln & "',"
	sql=sql & "offenderaddress='" & str & "',"
	sql=sql & "offendersuburb='" & subb & "',"
	sql=sql & "offenderPostcode=" & post & ","
	sql=sql & "offenderstate='" & st & "',"
	sql=sql & "offenderemail='" & ema & "',"
	sql=sql & "licencenumber='" & lic & "',"	
	sql=sql & "offenderphone='" & ph & "'"
...
<div class="art-post-inner art-article">
							...
							Offence Details have been Updated 
                                </h2>
                <div class="cleared"></div>
                                <div class="art-postcontent">
									
									
							<p>The offence details have been successfully updated.<br><br>
								Here are the updated details:<br><br>
								
								<div class="row">
								<span class="label">Offence ID</span><span class="formw"> <%= id %> </span>
								</div>
																						
								<div class="row">
								<span class="label">Offence Name</span><span class="formw"><%= offname %></span>
								</div>
								
								<div class="row">
								<span class="label">Offence Suburb</span><span class="formw"><%= offsub %></span>
								</div>
								
								<div class="row">
								<span class="label">Offence Date</span><span class="formw"><%= offdate %></span>
								</div>
								
								<div class="row">
								<span class="label">Status:</span><span class="formw">  <%= stat %></span>
								</div>		
								
								<div class="row">
								<span class="label">Offender Name:</span><span class="formw"> <%= fn %> &nbsp <%= ln %> </span>
								</div>		
